...
IMG ?= ghcr.io/onpier-oss/gcp-kafka-auth-operator

# Image URL for topic-access e2e test
E2E_TEST_TOPIC_ACCESS_IMG ?= ghcr.io/onpier-oss/gcp-kafka-test-topic-access
E2E_TEST_AUTH_PROXY_IMG ?= ghcr.io/onpier-oss/gcp-kafka-auth-proxy
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name gcp-kafka-auth-operator-builder
	$(CONTAINER_TOOL) buildx use gcp-kafka-auth-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm gcp-kafka-auth-operator-builder
	rm Dockerfile.cross
